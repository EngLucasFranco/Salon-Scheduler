...
const formularioInicial = { nome: '', tipo: 'servico', valor: '', custo: 0, impostos: 0, outros: 0, duracaoMinutos: 30 };
...
function CampoMoeda({ centavos, onChange, ...props }) {
  const referencia = useRef(null); const digitosDepoisCursor = useRef(null); const valor = moedaDeCentavos(centavos);
  useLayoutEffect(() => {
    const input = referencia.current; const digitos = digitosDepoisCursor.current;
    if (!input || digitos === null) return;
    let vistos = 0; let posicao = 0;
    for (let indice = valor.length - 1; indice >= 0; indice -= 1) { if (/\d/.test(valor[indice])) vistos += 1; if (vistos >= digitos) { posicao = indice; break; } }
    input.setSelectionRange(digitos === 0 ? valor.length : posicao, digitos === 0 ? valor.length : posicao); digitosDepoisCursor.current = null;
  }, [valor]);
  function alterar(evento) { const cursor = evento.target.selectionStart || 0; digitosDepoisCursor.current = evento.target.value.slice(cursor).replace(/\D/g, '').length; onChange(Number(evento.target.value.replace(/\D/g, '')) || 0); }
...
export default function Catalogo() {
  ...
  function custoTotal(custo = detalheCusto, impostos = detalheImpostos, outros = detalheOutros) { return Number(custo || 0) + Number(impostos || 0) + Number(outros || 0); }
  function calcularPreco(custo, impostos, outros, margem) { return Math.round(custoTotal(custo, impostos, outros) * (1 + Number(margem || 0) / 100)); }
  function margemDoProduto(custo, impostos, outros, preco) { const total = custoTotal(custo, impostos, outros); return total > 0 ? String(Math.max(0, Math.min(999, Math.round(((preco / total) - 1) * 100)))) : ''; }
  function abrirDetalhamento(origem = 'novo') { const produto = origem === 'edicao' ? formEdicao : form; const custo = valorParaCentavos(produto.custo); const impostos = valorParaCentavos(produto.impostos); const outros = valorParaCentavos(produto.outros); const preco = valorParaCentavos(produto.valor); setOrigemDetalhamento(origem); setDetalheNome(produto.nome); setDetalheCusto(custo); setDetalheImpostos(impostos); setDetalheOutros(outros); setDetalheMargem(margemDoProduto(custo, impostos, outros, preco)); setDetalhePreco(preco); setDetalhamentoAberto(true); }
  function alterarCusto(custo) { setDetalheCusto(custo); setDetalhePreco(calcularPreco(custo, detalheImpostos, detalheOutros, detalheMargem)); }
  function alterarImpostos(impostos) { setDetalheImpostos(impostos); setDetalhePreco(calcularPreco(detalheCusto, impostos, detalheOutros, detalheMargem)); }
  function alterarOutros(outros) { setDetalheOutros(outros); setDetalhePreco(calcularPreco(detalheCusto, detalheImpostos, outros, detalheMargem)); }
  function alterarMargem(valor) { const margem = valor.replace(/\D/g, '').slice(0, 3); setDetalheMargem(margem); setDetalhePreco(calcularPreco(detalheCusto, detalheImpostos, detalheOutros, margem)); }
  function atualizarMargem(preco) { setDetalheMargem(margemDoProduto(detalheCusto, detalheImpostos, detalheOutros, preco)); }
  ...
  function confirmarDetalhamento() { const valores = { nome: detalheNome, custo: (detalheCusto / 100).toFixed(2), impostos: (detalheImpostos / 100).toFixed(2), outros: (detalheOutros / 100).toFixed(2), valor: (detalhePreco / 100).toFixed(2) }; if (origemDetalhamento === 'edicao') setFormEdicao((anterior) => ({ ...anterior, ...valores })); else setForm((anterior) => ({ ...anterior, ...valores })); setDetalhamentoAberto(false); }
  ...
  function abrirEdicao(servico) {
    ...
    setFormEdicao({ nome: servico.nome, tipo: servico.tipo || 'servico', valor: servico.valor ?? '', custo: servico.custo ?? 0, impostos: servico.impostos ?? 0, outros: servico.outros ?? 0, duracaoMinutos: servico.duracaoMinutos || 30 });
    setErro('');
  }
