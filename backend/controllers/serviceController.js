...
function validarServico({ nome, tipo, valor, custo, duracaoMinutos }) {
...
async function criar(req, res) {
  try {
    ...
    const servico = await createService({ nome: req.body.nome.trim(), tipo: req.body.tipo, valor: Number(req.body.valor), custo: req.body.tipo === 'produto' ? Number(req.body.custo || 0) : 0, duracaoMinutos: req.body.tipo === 'servico' ? Number(req.body.duracaoMinutos) : null, criadoPor: req.usuario.id });
...
async function atualizar(req, res) {
  try {
    const erroValidacao = validarServico(req.body);
    if (erroValidacao) return res.status(400).json({ mensagem: erroValidacao });
    const servico = await updateService(req.params.id, { nome: req.body.nome.trim(), tipo: req.body.tipo, valor: Number(req.body.valor), custo: req.body.tipo === 'produto' ? Number(req.body.custo || 0) : 0, duracaoMinutos: req.body.tipo === 'servico' ? Number(req.body.duracaoMinutos) : null });
