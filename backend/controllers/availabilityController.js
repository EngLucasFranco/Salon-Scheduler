const { findAgenda, listAgendas, saveAgenda, newSlot } = require('../config/store');
...
async function abrirAgenda(req, res) {
  try {
    ...
    if (!agenda) agenda = { data, aberta: true, criadoPor: req.usuario.id, slots: [] };
    const existentes = new Set(agenda.slots.map((slot) => slot.horario));
    horarios.filter((horario) => !existentes.has(horario)).forEach((horario) => agenda.slots.push(newSlot(horario)));
...
module.exports = { abrirAgenda, fecharAgenda, removerSlot, bloquearSlot, cancelarReservaGestor, listarAgendaCompleta, listarPorData, reservarSlot, cancelarPropriaReserva, minhasReservas };
